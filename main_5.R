...
scatter_norm_counts <- function(labeled_results, dds_obj, num_genes)
  ...
  scatter_norm <- top_genes %>% 
    ...
    ggplot() + 
    geom_point(aes(x=genes, y=log10(norm_counts), 
                   color=samplenames), position=position_jitter(w=0.1,h=0)) + 
    theme_minimal() +  
    theme(axis.text.x=element_text(angle=90, hjust=1)) + 
    xlab(' ') +
    ggtitle('Plot of Log10(normalized counts) for top ten DE genes') +
    coord_flip() #flipped the axis for better plot
  return(scatter_norm)
}
...
run_gsea <- function(labeled_results, gmt, min_size, max_size)
  ...
  labeled_results <- labeled_results %>%
    separate(genes, sep='\\.', into='genes', remove=TRUE)
  ...
  hgnc_symbols <- read.csv("biomart_ensmusg_to_hgnc.csv")
  
   hgnc_results <- labeled_results %>% 
    left_join(hgnc_symbols, by=c('genes' = 'Gene.stable.ID'))
  ...
  ranks <- hgnc_results %>% 
    ...
    deframe()
  c_pathway <- gmtPathways('c2.cp.v7.5.1.symbols.gmt')
  
  fgsea_results <- fgsea(c_pathway, ranks, 
...
top_pathways <- function(fgsea_results, num_paths)
  {
  top_p <- fgsea_results %>% slice_max(NES, n=num_paths) %>% pull(pathway)
  top_n <- fgsea_results %>% slice_min(NES, n=num_paths) %>% pull(pathway)
  
  subset <- fgsea_results %>% 
    filter(pathway %in% c(top_p, top_n)) %>%
