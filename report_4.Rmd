...
pivot_longer(library_sizes/1e6, everything(), names_to="Sample",values_to="# reads") %>%
  ...
  ggtitle("Number of reads in millions per sample")
...
plot_sample_distributions(s_data, TRUE, "Sample Distribution of Log10(Raw Counts)")
```
...
plot_variance_vs_mean(s_data, TRUE, "Variance vs. Mean with a Log10(Raw Counts)")
```
...
plot_pca(s_data, meta, "PCA plaot with RAW COUNTS")
```
...
plot_sample_distributions(s_cpm, TRUE, "Sample Distribution with a log10(CPM)")
plot_variance_vs_mean(s_cpm, TRUE, "Variance vs Mean with a log10(CPM)")
plot_pca(s_cpm, meta, "CPM PCA Comparison")
```
