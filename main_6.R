...
load_n_trim <- function(filename) {
  count_data <- read.delim(filename, sep='\t', row.names="gene")
  result <- count_data[,c("vP0_1", "vP0_2", "vAd_1", "vAd_2")]
  return(result)
}
...
run_limma <- function(counts_dataframe, design, group) 
  ...
  fit <- lmFit(v, design)
  fit <- eBayes(fit)
  return(topTable(fit, coef = ncol(design), number = 15026, resort.by = "p"))
}
...
combine_pval <- function(deseq, edger, limma) {
  out <- tibble(deseq=deseq$pvalue,edger=edger$PValue,limma=limma$P.Value)
  result <- gather(out,key="package",value="pval",deseq,edger,limma)
  return(result)
}
...
create_facets <- function(deseq, edger, limma) {
  deseq.sub <- tibble(logFC=deseq$log2FoldChange,padj=deseq$padj)
  deseq.sub$package <- "deseq"
  edger.sub <- tibble(logFC=edger$logFC,padj=edger$padj)
  edger.sub$package <- "edger"
  limma.sub <- tibble(logFC=limma$logFC,padj=limma$adj.P.Val)
  limma.sub$package <- "limma"
  return(rbind(deseq.sub, edger.sub, limma.sub))
}
...
theme_plot <- function(volcano_data) {
  data <- cbind.data.frame('logFC'=as.numeric(volcano_data$logFC),
                           'padj'=-log10(as.numeric(volcano_data$padj)),
                           'package'=volcano_data$package)
  plot <- ggplot(data=data, aes(x=logFC, y=padj, color= padj>100))+
    geom_point() +
    theme_minimal() +
  ...
  plot$labels$colour <- "P-adj < 1e-100"
  return(plot)
}