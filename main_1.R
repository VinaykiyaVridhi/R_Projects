...
knitr::kable(head(metadata)[c(1:5,18)])
return()
...
rename_and_select <- function(data) {
  rename(data,Age = Age_at_diagnosis, Batch = normalizationcombatbatch , 
         Subtype = SixSubtypesClassification) %>%
    select(Sex, Age, TNM_Stage,Tumor_Location, 
...
summarize_expression <- function(expr_mat) {
  probe_avg <- apply(dplyr::select(expr_mat,-subject_id),2,mean)
  probe_var <- apply(dplyr::select(expr_mat,-subject_id),2,var)
  summarized <- tibble(
    "mean_exp" = probe_avg,
    "variance" = probe_var,
